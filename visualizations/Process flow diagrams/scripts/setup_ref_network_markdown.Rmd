---
title: "VisNetwork Flow Diagram Markdown"
...
# Create standardized "to" and "from" fields for table relationships,
# applying special formatting rules and removing duplicates.
metadata_df <- metadata_df %>%
  mutate(
    # Construct "to" field
    to = case_when(
      # For special schemas, use dot separator(customize these as needed)
      grepl("ai_|lhj|COVID_Staging", schema) ~ paste(schema, table, sep = "."),
      # If parent_table is missing, set to NA
      is.na(parent_table) ~ NA,
      # Otherwise, use pipe separator
...
# Set up nodes for network visualization.
# 
# Each node represents a table, standardized from both "to" and "from" fields.
# We assign groups, levels, and unique IDs to control how the network is drawn.
# 
# NOTE: A custom ordering is applied below because Visnetwork is not quite as good 
#       at organizing hierarchies in more complex scenarios. 
#       It's nice to have a standard hierarchical placement for certain schemas.

nodes_vis <- metadata_df %>%
  # Get unique "to" nodes
  distinct(to, schema, directory, script, data_source) %>%
  rename(label = to) %>%
  # Append unique "from" nodes
  bind_rows(
    distinct(metadata_df, from, parent_schema, parent_directory, script, data_source) %>% 
      filter(!is.na(from)) %>%
      rename(label = from, schema = parent_schema, directory = parent_directory)
  ) %>%
  # Assign groups (based on data_source) and hierarchical levels (based on schema)
  mutate(
    group = factor(data_source, levels = unique(data_source)),
    level = case_when(
      schema == "raw"     ~ 2,
      schema %in% "staging" ~ 3,
      schema %in% "prod"    ~ 4,
      schema %in% "output"  ~ 5,
      TRUE ~ 1   # fallback for any unexpected schemas
    )
  ) %>%
  arrange(group, script, schema) %>%
  # NOTE: if a table exists in multiple data sources, group is taken
  #       from the first occurrence in data_source order
  distinct(label, schema, .keep_all = TRUE) %>% 
  # Assign unique IDs for visualization
  mutate(id = seq(1, nrow(.))) %>% 
  # Keep only necessary fields for network visualization
...
# Set up edges for network visualization.
# Each edge represents a process between nodes
# usually a script that creates one table output from an input
edges_vis <- edges_vis %>%
  select(from, to
         , label
         , url, title)
