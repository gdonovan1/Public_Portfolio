...
metadata_df0 <- read_csv(file.path("C:/GitHub/Data_Pipeline/Documentation/ref.mock_table_dependencies.csv"))

unique_datasources <- unique(metadata_df0$data_source)
unique_datasources <- unique_datasources[!grepl("/", unique_datasources)]
# unique_datasources <- "ESSENCE" # To output a single datasource
...
for (source in c(unique_datasources, "ALL")) {
  ...
  output_filename <- paste0("flow_diagram_", source, ".html")
  rmarkdown::render(
    input = file.path(path_setup, "setup_ref_network_markdown.Rmd"),
    output_file = file.path(path_output, subfolder, output_filename),
...
for (source in c(unique_datasources)) {
  ...
  image_output <- file.path(path_setup, "flow_diagrams", subfolder, output_filename)
  temp_html <- tempfile(fileext = ".html")
  
  graph <- rmarkdown::render(
    input = file.path(path_setup, "setup_ref_flow_markdown.Rmd"),
